        ...
        case 'indexeddb':
          {
            ...
            if (this.component.findDatabase && this.component.findTable) {
              var request = window.indexedDB.open(this.component.findDatabase, 3);

              request.onupgradeneeded = function (event) {
                if (_this4.component.customOptions) {
                  var db = event.target.result;
                  var objectStore = db.createObjectStore(_this4.component.findTable, {
                    keyPath: 'myKey',
                    autoIncrement: true
                  });

                  objectStore.transaction.oncomplete = function () {
                    var transaction = db.transaction(_this4.component.findTable, 'readwrite');

                    _this4.component.customOptions.forEach(function (item) {
                      transaction.objectStore(_this4.component.findTable).put(item);
                    });
                  };
                }
              };
              ...
              request.onsuccess = function (event) {
                var db = event.target.result;
                var transaction = db.transaction(_this4.component.findTable, 'readwrite');
                var objectStore = transaction.objectStore(_this4.component.findTable);
